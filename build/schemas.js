...
export const ReadMessagesSchema = z.object({
    ...
    after: z.string().optional(), // Cursor for pagination (message ID)
    before: z.string().optional(), // Cursor for pagination (message ID)
    around: z.string().optional() // Cursor for pagination (message ID)
});
export const GetServerInfoSchema = z.object({
    guildId: z.string(),
    includeChannels: z.boolean().optional().default(true),
    channelsLimit: z.number().min(1).max(500).optional().default(100),
    channelsAfter: z.string().optional() // Cursor for pagination (channel ID)
});
...
export const ListMembersSchema = z.object({
    ...
    after: z.string().optional(), // Cursor for pagination (user ID)
    before: z.string().optional(), // Cursor for pagination (user ID)
...
export const ListRolesSchema = z.object({
    guildId: z.string(),
    limit: z.number().min(1).max(1000).optional().default(100),
    after: z.string().optional(), // Cursor for pagination (role ID)
    before: z.string().optional() // Cursor for pagination (role ID)
});
