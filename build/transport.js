import { StreamableHTTPServerTransport } from "@modelcontextprotocol/sdk/server/streamableHttp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import express from "express";
import { toolList } from './toolList.js';
import { createToolContext, loginHandler, sendMessageHandler, getForumChannelsHandler, createForumPostHandler, getForumPostHandler, replyToForumHandler, deleteForumPostHandler, createTextChannelHandler, deleteChannelHandler, readMessagesHandler, getServerInfoHandler, addReactionHandler, addMultipleReactionsHandler, removeReactionHandler, deleteMessageHandler, createWebhookHandler, sendWebhookMessageHandler, editWebhookHandler, deleteWebhookHandler, editCategoryHandler, createCategoryHandler, deleteCategoryHandler, bulkDeleteMessagesHandler, editMessageHandler, pinMessageHandler, unpinMessageHandler, moveMessagesHandler, listMembersHandler, getMemberHandler, listRolesHandler, createRoleHandler, editRoleHandler, deleteRoleHandler, assignRoleHandler, removeRoleHandler, setChannelPermissionsHandler, getChannelPermissionsHandler, removeChannelPermissionsHandler, editChannelHandler, createVoiceChannelHandler } from './tools/tools.js';
import { Client, GatewayIntentBits } from "discord.js";
import { info, error, sanitizeForLog } from './logger.js';
import { attemptReconnect } from './tools/middleware.js';
import { getRateLimiter, Priority, resetRateLimiter } from './rateLimiter.js';
import { resetCache } from './cache.js';
export class StdioTransport {
    transport = null;
    async start(server) {
        this.transport = new StdioServerTransport();
        await server.connect(this.transport);
    }
    async stop() {
        if (this.transport) {
            await this.transport.close();
            this.transport = null;
        }
    }
}

/**
 * Centralized tool handler registry.
 * Maps tool names to their handler functions.
 */
const TOOL_HANDLERS = {
    discord_login: loginHandler,
    discord_send: sendMessageHandler,
    discord_get_forum_channels: getForumChannelsHandler,
    discord_create_forum_post: createForumPostHandler,
    discord_get_forum_post: getForumPostHandler,
    discord_reply_to_forum: replyToForumHandler,
    discord_delete_forum_post: deleteForumPostHandler,
    discord_create_text_channel: createTextChannelHandler,
    discord_delete_channel: deleteChannelHandler,
    discord_edit_channel: editChannelHandler,
    discord_read_messages: readMessagesHandler,
    discord_get_server_info: getServerInfoHandler,
    discord_add_reaction: addReactionHandler,
    discord_add_multiple_reactions: addMultipleReactionsHandler,
    discord_remove_reaction: removeReactionHandler,
    discord_delete_message: deleteMessageHandler,
    discord_bulk_delete_messages: bulkDeleteMessagesHandler,
    discord_edit_message: editMessageHandler,
    discord_pin_message: pinMessageHandler,
    discord_unpin_message: unpinMessageHandler,
    discord_move_messages: moveMessagesHandler,
    discord_create_webhook: createWebhookHandler,
    discord_send_webhook_message: sendWebhookMessageHandler,
    discord_edit_webhook: editWebhookHandler,
    discord_delete_webhook: deleteWebhookHandler,
    discord_create_category: createCategoryHandler,
    discord_edit_category: editCategoryHandler,
    discord_delete_category: deleteCategoryHandler,
    discord_list_members: listMembersHandler,
    discord_get_member: getMemberHandler,
    discord_list_roles: listRolesHandler,
    discord_create_role: createRoleHandler,
    discord_edit_role: editRoleHandler,
    discord_delete_role: deleteRoleHandler,
    discord_assign_role: assignRoleHandler,
    discord_remove_role: removeRoleHandler,
    discord_set_channel_permissions: setChannelPermissionsHandler,
    discord_get_channel_permissions: getChannelPermissionsHandler,
    discord_remove_channel_permissions: removeChannelPermissionsHandler,
    discord_create_voice_channel: createVoiceChannelHandler,
};

/**
 * Ensures the Discord client is ready, attempting reconnect if needed.
 *
 * @param {Object} client - Discord.js client instance
 * @returns {Promise<boolean>} True if client is ready, false otherwise
 */
async function ensureClientReady(client) {
    if (client.isReady()) {
        return true;
    }

    // Log client state for debugging (sanitized)
    error('Client not ready, client state: ' + sanitizeForLog({
        isReady: client.isReady(),
        hasToken: !!client.token,
        user: client.user ? {
            id: client.user.id,
            tag: client.user.tag,
        } : null
    }));

    // Attempt to reconnect if we have a token
    if (client.token) {
        const reconnected = await attemptReconnect(client);
        if (reconnected) {
            info("Reconnected successfully, continuing with original request");
            return true;
        }
    }

    return false;
}

/**
 * Priority mapping for Discord tools
 * Maps tool names to their execution priority
 */
const TOOL_PRIORITIES = {
    // High priority: Critical operations
    'discord_login': Priority.HIGH,

    // Normal priority: Standard operations
    'discord_send': Priority.NORMAL,
    'discord_get_forum_channels': Priority.NORMAL,
    'discord_get_forum_post': Priority.NORMAL,
    'discord_read_messages': Priority.NORMAL,
    'discord_get_server_info': Priority.NORMAL,
    'discord_get_channel_permissions': Priority.NORMAL,
    'discord_list_members': Priority.NORMAL,
    'discord_get_member': Priority.NORMAL,
    'discord_list_roles': Priority.NORMAL,

    // Low priority: Bulk operations and writes
    'discord_create_forum_post': Priority.LOW,
    'discord_reply_to_forum': Priority.LOW,
    'discord_delete_forum_post': Priority.LOW,
    'discord_create_text_channel': Priority.LOW,
    'discord_delete_channel': Priority.LOW,
    'discord_edit_channel': Priority.LOW,
    'discord_create_category': Priority.LOW,
    'discord_edit_category': Priority.LOW,
    'discord_delete_category': Priority.LOW,
    'discord_create_voice_channel': Priority.LOW,
    'discord_add_reaction': Priority.LOW,
    'discord_add_multiple_reactions': Priority.LOW,
    'discord_remove_reaction': Priority.LOW,
    'discord_delete_message': Priority.LOW,
    'discord_bulk_delete_messages': Priority.LOW,
    'discord_edit_message': Priority.LOW,
    'discord_pin_message': Priority.LOW,
    'discord_unpin_message': Priority.LOW,
    'discord_move_messages': Priority.LOW,
    'discord_create_webhook': Priority.LOW,
    'discord_send_webhook_message': Priority.LOW,
    'discord_edit_webhook': Priority.LOW,
    'discord_delete_webhook': Priority.LOW,
    'discord_create_role': Priority.LOW,
    'discord_edit_role': Priority.LOW,
    'discord_delete_role': Priority.LOW,
    'discord_assign_role': Priority.LOW,
    'discord_remove_role': Priority.LOW,
    'discord_set_channel_permissions': Priority.LOW,
    'discord_remove_channel_permissions': Priority.LOW,
};

/**
 * Centralized tool dispatcher function.
 * Handles both legacy format (discord_send) and new MCP format (tools/call).
 * Uses rate limiter for all non-login operations.
 *
 * @param {string} toolName - Name of the tool to execute
 * @param {Object} args - Arguments to pass to the tool handler
 * @param {Object} toolContext - Tool context containing Discord client
 * @returns {Promise<Object>} Result from the tool handler
 * @throws {Error} If tool not found or client not ready
 */
async function dispatchTool(toolName, args, toolContext) {
    const handler = TOOL_HANDLERS[toolName];
    if (!handler) {
        throw new Error(`Unknown tool: ${toolName}`);
    }

    // Check if client is ready (except for login)
    if (toolName !== 'discord_login') {
        const isReady = await ensureClientReady(toolContext.client);
        if (!isReady) {
            throw new Error('Discord client not logged in. Please use discord_login tool first.');
        }
    }

    // Get the priority for this tool (default to NORMAL)
    const priority = TOOL_PRIORITIES[toolName] || Priority.NORMAL;

    // Use rate limiter for all operations
    const rateLimiter = getRateLimiter();

    // For login, execute directly without rate limiting
    if (toolName === 'discord_login') {
        return await handler(args, toolContext);
    }

    // Execute through rate limiter with appropriate priority
    return await rateLimiter.execute(
        () => handler(args, toolContext),
        priority,
        { toolName }
    );
}
export class StreamableHttpTransport {
    port;
    app;
    server = null;
    httpServer = null;
    transport = null;
    toolContext = null;
    sessionId = '';
    constructor(port = 8080) {
        this.port = port;
        this.app = express();
        this.app.use(express.json());
        this.setupEndpoints();
        this.sessionId = `session_${Date.now()}_${Math.random().toString(36).substring(2, 10)}`;
        info(`Created HTTP transport with session ID: ${this.sessionId}`);
    }
    setupEndpoints() {
        // Handler for POST requests
        this.app.post('/mcp', (req, res) => {
            info('Received MCP request: ' + sanitizeForLog(req.body));
            this.handleMcpRequest(req, res).catch(err => {
                error('Unhandled error in MCP request: ' + String(err));
            });
        });
        // Handler for non-POST methods
        this.app.all('/mcp', (req, res) => {
            if (req.method !== 'POST') {
                res.status(405).json({
                    jsonrpc: '2.0',
                    error: {
                        code: -32000,
                        message: 'Method not allowed. Use POST.',
                    },
                    id: null,
                });
            }
        });
    }
    async handleMcpRequest(req, res) {
        try {
            if (!this.server) {
                return res.json({
                    jsonrpc: '2.0',
                    error: {
                        code: -32603,
                        message: 'Server not initialized',
                    },
                    id: req.body?.id || null,
                });
            }
            info(`Request body (session ${this.sessionId}): ${sanitizeForLog(req.body)}`);
            // Handle all tool requests in a generic way
            if (!req.body.method) {
                return res.status(400).json({
                    jsonrpc: '2.0',
                    error: {
                        code: -32600,
                        message: 'Invalid Request: No method specified',
                    },
                    id: req.body?.id || null,
                });
            }
            // Handle all tools directly with proper error handling
            try {
                const method = req.body.method;
                const params = req.body.params || {};
                // Make sure toolContext is available for tool methods
                if (!this.toolContext && method !== 'list_tools' && method !== 'initialize') {
                    return res.status(400).json({
                        jsonrpc: '2.0',
                        error: {
                            code: -32603,
                            message: 'Tool context not initialized. Service may need to be restarted.',
                        },
                        id: req.body?.id || null,
                    });
                }
                let result;
                // Handle each tool method directly
                switch (method) {
                    case 'initialize':
                        // Handle initialize method for MCP protocol compliance
                        result = {
                            protocolVersion: "2025-03-26",
                            capabilities: {
                                tools: {
                                    listChanged: false
                                },
                                logging: {}
                            },
                            serverInfo: {
                                name: "MCP-Discord",
                                version: "1.2.0"
                            }
                        };
                        break;
                    case 'notifications/initialized':
                        // Client indicates it's ready to begin normal operation
                        info("Client initialized. Starting normal operations.");
                        // No result needed for notifications
                        return res.json({
                            jsonrpc: "2.0",
                            result: null,
                            id: req.body.id
                        });
                    case 'tools/list':
                        // New MCP method name format
                        result = { tools: toolList };
                        break;
                    case 'list_tools':
                        // Legacy method name for backward compatibility
                        result = { tools: toolList };
                        break;
                    case 'discord_login':
                        result = await dispatchTool(method, params, this.toolContext);
                        // Log client state after login (sanitized)
                        info('Client state after login: ' + sanitizeForLog({
                            isReady: this.toolContext.client.isReady(),
                            hasToken: !!this.toolContext.client.token,
                            user: this.toolContext.client.user ? {
                                id: this.toolContext.client.user.id,
                                tag: this.toolContext.client.user.tag,
                            } : null
                        }));
                        break;
                    // Handle all Discord tool methods using centralized dispatcher
                    case 'discord_send':
                    case 'discord_get_forum_channels':
                    case 'discord_create_forum_post':
                    case 'discord_get_forum_post':
                    case 'discord_reply_to_forum':
                    case 'discord_delete_forum_post':
                    case 'discord_create_text_channel':
                    case 'discord_delete_channel':
                    case 'discord_edit_channel':
                    case 'discord_read_messages':
                    case 'discord_get_server_info':
                    case 'discord_add_reaction':
                    case 'discord_add_multiple_reactions':
                    case 'discord_remove_reaction':
                    case 'discord_delete_message':
                    case 'discord_bulk_delete_messages':
                    case 'discord_edit_message':
                    case 'discord_pin_message':
                    case 'discord_unpin_message':
                    case 'discord_move_messages':
                    case 'discord_create_webhook':
                    case 'discord_send_webhook_message':
                    case 'discord_edit_webhook':
                    case 'discord_delete_webhook':
                    case 'discord_create_category':
                    case 'discord_edit_category':
                    case 'discord_delete_category':
                    case 'discord_list_members':
                    case 'discord_get_member':
                    case 'discord_list_roles':
                    case 'discord_create_role':
                    case 'discord_edit_role':
                    case 'discord_delete_role':
                    case 'discord_assign_role':
                    case 'discord_remove_role':
                    case 'discord_set_channel_permissions':
                    case 'discord_get_channel_permissions':
                    case 'discord_remove_channel_permissions':
                    case 'discord_create_voice_channel':
                        result = await dispatchTool(method, params, this.toolContext);
                        break;
                    case 'tools/call':
                        // Handle new tools/call method format
                        const toolName = params.name;
                        const toolArgs = params.arguments || {};
                        result = await dispatchTool(toolName, toolArgs, this.toolContext);
                        break;
                    default:
                        // For method 'ping' and other non-critical methods, just return an empty result
                        // This ensures MCP compatibility for health checks and probes
                        if (method === 'ping') {
                            info(`Returning empty response for ping request`);
                            result = {};
                        }
                        else {
                            // For other unknown methods, return method not found error
                            return res.status(400).json({
                                jsonrpc: '2.0',
                                error: {
                                    code: -32601,
                                    message: `Method not found: ${method}`,
                                },
                                id: req.body?.id || null,
                            });
                        }
                }
                info(`Request for ${method} handled successfully`);
                // Handle the case where tool handlers return { content, isError }
                if (result && typeof result === 'object' && 'content' in result) {
                    // If it's an error from the tool handler
                    if ('isError' in result && result.isError) {
                        error('Tool error response: ' + sanitizeForLog(result));
                        return res.json({
                            jsonrpc: '2.0',
                            id: req.body.id,
                            error: {
                                code: -32603,
                                message: Array.isArray(result.content)
                                    ? result.content.map((item) => item.text).join(' ')
                                    : 'Tool execution error'
                            }
                        });
                    }
                    // Return success result but maintain same format as other RPC methods
                    const finalResponse = {
                        jsonrpc: '2.0',
                        id: req.body.id,
                        result: result
                    };
                    info(`Sending response (session ${this.sessionId}): ${sanitizeForLog(finalResponse)}`);
                    return res.json(finalResponse);
                }
                // Standard result format
                const finalResponse = {
                    jsonrpc: '2.0',
                    id: req.body.id,
                    result: result
                };
                info(`Sending response (session ${this.sessionId}): ${sanitizeForLog(finalResponse)}`);
                return res.json(finalResponse);
            }
            catch (err) {
                error('Error processing tool request: ' + String(err));
                // Handle validation errors
                if (err && typeof err === 'object' && 'name' in err && err.name === 'ZodError') {
                    return res.json({
                        jsonrpc: '2.0',
                        error: {
                            code: -32602,
                            message: `Invalid parameters: ${err && typeof err === 'object' && 'message' in err ? String(err.message) : 'Unknown validation error'}`,
                        },
                        id: req.body?.id || null,
                    });
                }
                // Handle all other errors
                return res.json({
                    jsonrpc: '2.0',
                    error: {
                        code: -32603,
                        message: err instanceof Error ? err.message : 'Unknown error',
                    },
                    id: req.body?.id || null,
                });
            }
        }
        catch (err) {
            error('Error handling MCP request: ' + String(err));
            if (!res.headersSent) {
                res.json({
                    jsonrpc: '2.0',
                    error: {
                        code: -32603,
                        message: err instanceof Error ? err.message : 'Internal server error',
                    },
                    id: req.body?.id || null,
                });
            }
        }
    }
    async start(server) {
        this.server = server;
        info('Starting HTTP transport with server: ' + String(!!this.server));
        // Try to get client from the DiscordMCPServer instance
        // First, check if the server is passed from DiscordMCPServer
        if (server) {
            // Try to access client directly from server._context
            const anyServer = server;
            let client;
            if (anyServer._context?.client) {
                client = anyServer._context.client;
                info('Found client in server._context');
            }
            // Also check if the server object has client directly
            else if (anyServer.client instanceof Client) {
                client = anyServer.client;
                info('Found client directly on server object');
            }
            // Look in parent object if available
            else if (anyServer._parent?.client instanceof Client) {
                client = anyServer._parent.client;
                info('Found client in server._parent');
            }
            if (client) {
                this.toolContext = createToolContext(client);
                info('Tool context initialized with Discord client');
            }
            else {
                // Create a real Discord client instead of a dummy
                info('Creating new Discord client for transport');
                const newClient = new Client({
                    intents: [
                        GatewayIntentBits.Guilds,
                        GatewayIntentBits.GuildMessages,
                        GatewayIntentBits.MessageContent,
                        GatewayIntentBits.GuildMessageReactions
                    ]
                });
                this.toolContext = createToolContext(newClient);
                info('Tool context initialized with new Discord client');
            }
        }
        // Create a stateless transport
        this.transport = new StreamableHTTPServerTransport({
            sessionIdGenerator: undefined // set to undefined for stateless servers
        });
        // Connect the transport
        await this.server.connect(this.transport);
        info('Transport connected');
        return new Promise((resolve) => {
            this.httpServer = this.app.listen(this.port, '0.0.0.0', () => {
                info(`MCP Server listening on 0.0.0.0:${this.port}`);
                resolve();
            });
        });
    }
    async stop() {
        if (this.transport) {
            await this.transport.close();
            this.transport = null;
        }
        if (this.server) {
            await this.server.close();
            this.server = null;
        }
        if (this.httpServer) {
            return new Promise((resolve) => {
                this.httpServer.close(async () => {
                    info('HTTP server closed');
                    this.httpServer = null;
                    // Clean up rate limiter and cache on shutdown
                    const rateLimiter = getRateLimiter();
                    await rateLimiter.drain();
                    resetRateLimiter();
                    resetCache();
                    info('Rate limiter and cache cleaned up');
                    resolve();
                });
            });
        }
    }
}
